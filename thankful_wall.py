...
if 'last_scroll_update' not in st.session_state:
    st.session_state.last_scroll_update = time.time()
if 'scroll_enabled' not in st.session_state:
    st.session_state.scroll_enabled = True
if 'scroll_speed' not in st.session_state:
    st.session_state.scroll_speed = 1
...
if not entries:
    ...
    st.sidebar.header("Scroll Settings 滚动设置")
    
    # Toggle for scrolling
    scroll_enabled = st.sidebar.toggle("Enable Auto-Scroll 启用自动滚动", value=st.session_state.scroll_enabled)
    
    if scroll_enabled != st.session_state.scroll_enabled:
        st.session_state.scroll_enabled = scroll_enabled
        st.rerun()
    
    if scroll_enabled:
        # Scroll speed control
        scroll_speed = st.sidebar.slider("Scroll Speed 滚动速度", 0.5, 5.0, 1.0, 0.5)
        ...
        time_diff = current_time - st.session_state.last_scroll_update
        
        # Calculate new scroll position (pixels per second)
        scroll_increment = time_diff * (scroll_speed * 10)  # Adjust multiplier for sensitivity
        st.session_state.scroll_position += scroll_increment
        st.session_state.last_scroll_update = current_time
        
        # Reset scroll position if it goes too far (simulate continuous scroll)
        # We'll estimate the total height needed for all entries
        estimated_total_height = len(entries) * 200  # Approximate 200px per entry
        if st.session_state.scroll_position > estimated_total_height + 1000:
            st.session_state.scroll_position = -800  # Start above viewport for smooth loop
        
        # Apply scrolling with CSS
        scroll_style = f"""
        <style>
        .scrolling-container {{
            height: 600px;
        ...
        .scrolling-content {{
            position: absolute;
            top: -{st.session_state.scroll_position}px;
            width: 95%;
        ...
        st.markdown("### 🎪 Live Scrolling Display 实时滚动展示")
        ...
        # Display all entries in the scrolling container
        for entry_id, info in entries.items():
            entry_html = f"""
            <div class="scroll-entry">
                <div style="display: flex; justify-content: space-between; margin-bottom: 10px;">
                    <strong>English Name:</strong> {info['english_name']}
                    <strong>Chinese Name:</strong> {info['chinese_name']}
                    <strong>Class/Role:</strong> {info.get('role_class', 'Not specified 未指定')}
                </div>
                <div style="margin-bottom: 10px;">
                    <strong>Thankful For:</strong> {info['thankful_for']}
                </div>
                <div style="font-size: 0.8em; color: #666;">
                    {f"Position: {info['manual_order']} • " if info.get('manual_order') else ""}
                    Entry ID: {entry_id[:8]}...
                </div>
            </div>
            """
        ...
        st.markdown('</div>', unsafe_allow_html=True)  # Close scrolling-content
        st.markdown('</div>', unsafe_allow_html=True)  # Close scrolling-container
        
        # Show current scroll status
        st.caption(f"🔄 Auto-scrolling at {scroll_speed}x speed • 以{scroll_speed}倍速度自动滚动")
        
        # Manual scroll controls
        col_control1, col_control2, col_control3 = st.columns([1, 2, 1])
        with col_control1:
            if st.button("⏪ Slow Down 减速", key="slow_scroll"):
                st.session_state.scroll_speed = max(0.5, st.session_state.scroll_speed - 0.5)
                st.rerun()
        with col_control2:
            st.write(f"Current Speed: {st.session_state.scroll_speed}x • 当前速度")
        with col_control3:
            if st.button("Speed Up 加速 ⏩", key="fast_scroll"):
                st.session_state.scroll_speed = min(5.0, st.session_state.scroll_speed + 0.5)
                st.rerun()
        
        st.divider()
        st.subheader("All Entries (Static View) 所有条目 (静态视图)")
    
    # Display all entries in static view below the scrolling section
    for entry_id, info in entries.items():
        with st.container():
...
- **Auto-scrolling animations** - Continuous content display
...
# Force continuous updates for scrolling
if st.session_state.scroll_enabled:
